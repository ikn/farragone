...
with_warnings: whether to compute warnings that can only be determine while
...
Returns an iterator yielding `(rename, warnings)`, where `rename` is as yielded
by `get_renames` and `warnings` is a sequence of util.Warn instances.

"""
    for frm, to, warnings in _get_renames(True, *args, **kwargs):
